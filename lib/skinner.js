...
exports.p2Bucketizer = skP2Bucketize;
...
function skP2Bucketize(rv, value, card)
{
	var i, ent;
	...
	ent = [ [ 0, 0 ], card ];
	ent[0][0] = skLargestP2Leq(value);
	ent[0][1] = Math.max(2 * ent[0][0] - 1, ent[0][0]);
	rv.splice(i, 0, ent);
	return (rv);
}

function skLargestP2Leq(value)
{
	var thresh;
...
{
	return (function (rv, value, card) {
		return (skP2Bucketize(rv, value, card));
	});
}
