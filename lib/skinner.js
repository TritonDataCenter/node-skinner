 ...
 *
 *
 * EXPANDED VS. COMPACT FORM FOR DISTRIBUTIONS
 *
 * The above examples of quantized numeric values use the *expanded form*.  In
 * this form, each bucket is represented with a tuple describing the minimum and
 * maximum values in the bucket, along with the number of entries in the bucket.
 * For example:
 *
 *     [ [ 4, 7 ], 10378 ]
 *
 * denotes "10378 entries in the range between 4 and 7, inclusive".  This
 * expanded form is sparse in that any buckets with no entries will not be
 * represented, so you may have:
 *
 *     [
 *         [ [  4,  7 ], 10378 ],
 *         [ [ 16, 31 ], 352 ]
 ...
 *
 * However, for other use cases, particularly where a non-sparse representation
 * is needed (as when printing to a terminal), it's more useful to refer to each
 * bucket by its *ordinal* index.  A non-sparse representation of the above is:
 *
 *    [
 *        [ [ 0,  0 ],     0 ],
 *        [ [ 1,  1 ],     0 ],
 ...
 *    ]
 *
 * The *compact* representation looks like this:
 *
 *    [
 *        [ 3, 10378 ],
 *        [ 5, 352 ]
 ...
 *
 *    ordinalBuckets	If true, bucket ranges are represented with ordinal
 *    			indexes ("compact form"), instead of the actual bucket
 *    			range.  See "EXPANDED VS. COMPACT FORM FOR
 *    			DISTRIBUTIONS" above.
 *
...
function skAggregator(args)
{
	...
	this.sa_ordinal = args.ordinalBuckets ? true : false;
	...
	for (; i < this.sa_decomps.length; i++) {
		...
		mod_assert.equal('object',
		    typeof (this.sa_bucketizers[this.sa_decomps[i]]),
...
skAggregator.prototype._transform = function (datapt, _, callback)
{
	...
	if (i < this.sa_decomps.length) {
		...
		if (typeof (fieldvalue) == 'number' && !isNaN(fieldvalue)) {
			bucketizer.bucketize(o, fieldvalue, datapt['value']);
		} else {
			this.sa_nnonnumeric++;
			this.emit('invalid_object', datapt, new VError(
			    'value for field "%s" is not a number', field),
			    this.sa_nrecords);
		}
...
{
	if (!this.sa_aspoints) {
		this.push(this.resultRaw(this.sa_ordinal));
		callback();
		return;
	}

	var self, result, bucketizer;

	self = this;
	result = this.resultRaw(true);
	bucketizer = this.sa_ndiscrete == this.sa_decomps.length ?
	    null : this.sa_bucketizers[this.sa_decomps[this.sa_ndiscrete]];
	result.forEach(function (row) {
		...
		for (i = 0; i < self.sa_ndiscrete; i++)
			point.fields[self.sa_decomps[i]] = row[i];

		if (self.sa_decomps.length === 0) {
			/*
			 * If there are no decompositions at all, then the "row"
			 * is actually just a number.
			 */
			mod_assert.equal(typeof (row), 'number');
			point.value = row;
			self.push(point);
		} else if (i == self.sa_decomps.length) {
			/*
			 * If there are no numeric fields, then the last entry
			 * in "row" is the numeric value.
			 */
			...
			/*
			 * If there is a numeric breakdown then we'll emit a
			 * data point for each bucket that we find in this row.
			 */
			mod_assert.equal(i, self.sa_ndiscrete);
			mod_assert.equal(i, self.sa_decomps.length - 1);
			mod_assert.ok(bucketizer !== null);
			mod_assert.ok(i < row.length);
			for (bi = i; bi < row.length; bi++) {
				p = mod_jsprim.deepCopy(point);
				p.fields[self.sa_decomps[i]] =
				    bucketizer.bucketMin(row[bi][0]);
				p.value = row[bi][1];
				self.push(p);
			}
		}
...
{
	return (this.resultRaw(this.sa_ordinal));
};

skAggregator.prototype.resultRaw = function (ordinal)
{
	var self = this;
	var bucketizer, rv;

	rv = mod_jsprim.flattenObject(this.sa_value, this.sa_ndiscrete);
	if (ordinal || this.sa_ndiscrete == this.sa_decomps.length)
		return (rv);

	bucketizer = this.sa_bucketizers[this.sa_decomps[this.sa_ndiscrete]];
	mod_assert.ok(bucketizer);
	return (rv.map(function (row) {
		var rowrv, i, bmin, bmax;
		rowrv = row.slice(0);
		for (i = self.sa_ndiscrete; i < row.length; i++) {
			mod_assert.ok(Array.isArray(row[i]));
			mod_assert.equal(row[i].length, 2);
			bmin = bucketizer.bucketMin(row[i][0]);
			bmax = bucketizer.bucketMax(row[i][0]);
			rowrv[i] = [ [ bmin, bmax ], row[i][1] ];
		}
		return (rowrv);
	}));
};
