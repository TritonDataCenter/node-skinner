...
function skAggregator(args)
{
	...
	if (args) {
		mod_assertplus.optionalObject(args.bucketizers,
		    'args.bucketizers');
		mod_assertplus.optionalObject(args.streamOptions,
		    'args.streamOptions');
		mod_assertplus.optionalArrayOfString(args.decomps,
		    'args.decomps');
	} else {
		args = {};
	}
...
{
	mod_assertplus.object(datapt, 'datapt');
	mod_assertplus.number(datapt.value, 'datapt.value');
	mod_assertplus.object(datapt.fields, 'datapt.fields');
...
skAggregator.prototype.resultsAsPoints = function resultsAsPoints()
{
	...
	mod_jsprim.flattenIter(this.sa_value, this.sa_decomps.length,
		...
		mod_assertplus.equal(i, row.length - 1);
		mod_assertplus.number(row[i], 'row[i]');
		point.value = row[i];
		rv.push(point);
	    });
	return (rv);
...
{
	mod_assertplus.func(this._bucketIndexToMin,
	    'children of "skBucketizer" must implement _bucketIndexToMin');
	mod_assertplus.func(this._valueToBucketIndex,
...
skBucketizer.prototype.bucketMax = function bucketMax(i)
{
	...
	mod_assertplus.ok(min >= 0);
	nextmin = this._bucketIndexToMin(i + 1);
	mod_assertplus.ok(nextmin > min);
...
{
	mod_assertplus.array(rv, 'rv');
	mod_assertplus.number(value, 'value');
	mod_assertplus.number(card, 'card');
	...
	mod_assertplus.ok(i == rv.length || value < bmin);
	mod_assertplus.ok(i === 0 || value >= this.bucketMin(rv[i - 1][0] + 1));
	bidx = this._valueToBucketIndex(value);
	mod_assertplus.ok(value >= this.bucketMin(bidx));
	mod_assertplus.ok(value < this.bucketMin(bidx + 1));
	ent = [ bidx, card ];
	rv.splice(i, 0, ent);
	return (rv);
};
    ...
    function _llBucketIndexToMin(bidx)
{
	...
	mod_assertplus.ok(i + uniquebuckets > bidx);
	mod_assertplus.ok(i <= bidx);
...
function skOrdinalToBounds(bucketizer, dist)
{
	return (dist.map(function (row, i) {
		mod_assertplus.array(row, 'row');
		mod_assertplus.equal(row.length, 2);
		mod_assertplus.number(row[0], 'row[0]');
